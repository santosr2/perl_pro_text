package PerlText::Parser::Custom;
use v5.36;
use Moo;
...
PerlText::Parser::Custom - User-defined log parsers with regex patterns
...
=head1 SYNOPSIS
    ...
    my $haproxy = PerlText::Parser::Custom->create_common('haproxy');
    my $pg      = PerlText::Parser::Custom->create_common('postgresql');

    # Create from config hash (useful for YAML/JSON config files)
    my $parser = PerlText::Parser::Custom->from_config({
        name    => 'myformat',
