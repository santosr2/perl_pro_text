...
register_function('is_ip', sub ($str) {
    require Sift::Utils;
    return Sift::Utils::is_ipv4($str) || Sift::Utils::is_ipv6($str) ? 1 : 0;
...
register_function('domain', sub ($url) {
    require Sift::Utils;
    return Sift::Utils::extract_domain($url);
...
Sift::Query::Functions - Built-in query functions

=head1 SYNOPSIS

    use Sift::Query::Functions qw(call_function list_functions);
...
Provides built-in functions for use in Sift query expressions.
