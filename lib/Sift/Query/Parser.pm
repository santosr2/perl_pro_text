package Sift::Query::Parser;
use v5.36;
use Moo;
use Marpa::R2;
use Sift::Query::Grammar;
use Sift::Query::AST;
use namespace::autoclean;

has grammar => (
    is      => 'lazy',
    default => sub { Sift::Query::Grammar->new->grammar },
);

sub parse ($self, $query_string) {
    my $recce = Marpa::R2::Scanless::R->new({
        grammar         => $self->grammar,
        semantics_package => 'Sift::Query::Parser::Actions',
    });
...
package Sift::Query::Parser::Actions {
    use v5.36;
    ...
    sub build_query ($self, $where, $group, $aggs, $sort, $limit) {
        return Sift::Query::AST::Query->new(
            sources => [],
            where   => $where,
            group   => $group // [],
            aggs    => $aggs  // [],
    ...
    sub or_expr ($self, $left, $kw, $right) {
        return Sift::Query::AST::BinaryExpr->new(
            op    => 'OR',
            left  => $left,
            right => $right,
        );
    ...
    sub not_expr ($self, $kw, $operand) {
        return Sift::Query::AST::UnaryExpr->new(
            op      => 'NOT',
            operand => $operand,
        );
    }
    ...
    sub comparison ($self, $field, $op, $value) {
        return Sift::Query::AST::Comparison->new(
            field => $field,
            op    => $op,
            value => $value,
        );
    ...
    sub in_expr ($self, $field, $kw, $lb, $values, $rb) {
        return Sift::Query::AST::InExpr->new(
            field  => $field,
            values => $values,
        );
    }
    ...
    sub count_agg ($self, @args) {
        return Sift::Query::AST::Aggregation->new(func => 'count');
    }

    sub avg_agg ($self, $kw, $field) {
        return Sift::Query::AST::Aggregation->new(func => 'avg', field => $field);
    }

    sub sum_agg ($self, $kw, $field) {
        return Sift::Query::AST::Aggregation->new(func => 'sum', field => $field);
    }

    sub min_agg ($self, $kw, $field) {
        return Sift::Query::AST::Aggregation->new(func => 'min', field => $field);
    }

    sub max_agg ($self, $kw, $field) {
        return Sift::Query::AST::Aggregation->new(func => 'max', field => $field);
    }
...
Sift::Query::Parser - Parse query strings into AST
...
Parses Sift query language strings into an AST using Marpa::R2.
